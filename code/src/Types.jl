...
# ====================================================================== #
# Session 4 — intraday production engine: compliance queue, signed
# decisions, tomorrow's ticket, and signed ticket. These types support
# the 30-minute intraday cadence in `production_runner.jl --mode=engine`,
# the evening desk-review workflow, and the next-morning signed-ticket
# execution path.
# ====================================================================== #
...
A proposed trade that failed one or more compliance gates and was diverted
to the human-review queue instead of auto-executing. Each intraday cron
fire that produces a queued trade appends one of these to the day's queue
file (`data/queue/queue-YYYY-MM-DD.jld2`).

### Fields
- `id::String` — unique queue identifier (e.g., `"2026-05-11T11:30:00-AAPL-buy-3"`)
- `timestamp::DateTime` — when the engine proposed the trade
- `ticker::String` — symbol the trade references
- `qty::Int` — proposed share count (positive integer; `side` carries direction)
- `side::Symbol` — `:buy` or `:sell`
- `proposed_weight::Float64` — target portfolio weight the engine wanted
- `gate_violations::Vector{Symbol}` — which gates the trade failed
  (e.g., `[:concentration_cap, :position_size_limit]`)
- `engine_snapshot::Dict{String,Any}` — engine state at the time
  (EWLS state hash, bandit pulls, sentiment, lambda, regime) for review context
"""
mutable struct MyComplianceQueueItem
    ...
    qty::Int
    side::Symbol
    proposed_weight::Float64
    gate_violations::Vector{Symbol}
    engine_snapshot::Dict{String,Any}

    # constructor -
    MyComplianceQueueItem() = new();
end

"""
    MySignedDecision

A class (or PM) adjudication of one [`MyComplianceQueueItem`](@ref). The
evening review notebook produces one of these per queue item and writes
the bundle to `data/decisions/decisions-YYYY-MM-DD.jld2`.

### Fields
- `queue_id::String` — id of the queue item this decision adjudicates
- `action::Symbol` — `:approve`, `:reject`, or `:modify`
- `modified_qty::Union{Int,Nothing}` — new share count when `action == :modify`,
  otherwise `nothing`
- `notes::String` — free-form rationale recorded for audit
- `signed_by::String` — adjudicator identifier (e.g., `"Maya Chen"`)
- `signed_at::DateTime` — when the adjudication was recorded
"""
mutable struct MySignedDecision

    # data -
    queue_id::String
    action::Symbol
    modified_qty::Union{Int,Nothing}
    notes::String
    signed_by::String
    signed_at::DateTime

    # constructor -
    MySignedDecision() = new();
end

"""
    MyTomorrowsTicket

The engine's proposed allocation for tomorrow's market open, generated at
today's 16:00 close fire. The evening review notebook loads this, the
class optionally modifies it, and produces a [`MySignedTicket`](@ref).

### Fields
- `target_weights::Vector{Float64}` — proposed weight per ticker (sum ≤ 1)
- `tickers::Vector{String}` — column order for `target_weights`
- `proposed_trades::Vector{NamedTuple}` — concrete trade list
  (`(ticker, qty, side)`) needed to move from current positions to targets
- `sentiment::MySentimentSignal` — sentiment snapshot at ticket generation
- `news_flags::Vector{String}` — high-severity news items flagged at generation
- `generated_at::DateTime` — timestamp of ticket creation
- `eta_used::Float64` — CES elasticity selected by the bandit for the open
- `regime::Symbol` — sentiment regime (`:bear`, `:neutral`, `:bull`)
"""
mutable struct MyTomorrowsTicket

    # data -
    target_weights::Vector{Float64}
    tickers::Vector{String}
    proposed_trades::Vector{NamedTuple}
    ...
    # constructor -
    MyTomorrowsTicket() = new();
end

"""
    MySignedTicket

A class-signed wrapper around a [`MyTomorrowsTicket`](@ref). The next-day
9:35am cron loads `data/tickets/signed-YYYY-MM-DD.jld2` and submits the
underlying trades, applying any modifications recorded here.

### Fields
- `ticket::MyTomorrowsTicket` — the underlying ticket as generated by the engine
- `modifications::Vector{NamedTuple}` — class-applied edits
  (`(ticker, original_qty, modified_qty, reason)`); empty if signed as-is
- `signed_by::String` — sign-off identifier
- `signed_at::DateTime` — sign-off timestamp
"""
mutable struct MySignedTicket

    # data -
    ticket::MyTomorrowsTicket
    modifications::Vector{NamedTuple}
    signed_by::String
    signed_at::DateTime
